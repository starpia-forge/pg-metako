...
- ✅ **자동 페어 모드**: 2노드 클러스터를 위한 원활한 자동 페일오버
...
pg-metako는 이제 2노드 클러스터를 자동으로 감지하고 원활한 페어 모드 기능으로 최적화합니다:

#### 페어 모드 특징
- **자동 감지**: 특별한 설정 불필요 - 2노드 클러스터에서 페어 모드 자동 활성화
- **원활한 페일오버**: 마스터 노드 장애 시 슬레이브를 자동으로 마스터로 승격
- **안전 메커니즘**: 스플릿 브레인 시나리오 방지를 위한 다중 안전 장치
- **실패 임계값**: 페일오버 트리거 전 설정 가능한 연속 실패 횟수
- **안전 지연**: 일시적 문제 방지를 위한 페일오버 실행 전 설정 가능한 지연
- **복구 감지**: 안전 지연 중 장애 노드 복구 시 자동 페일오버 취소

#### 페어 모드 안전 메커니즘
1. **연속 실패 임계값**: 페일오버 전 필요한 설정 가능한 연속 실패 횟수
2. **안전 지연**: 일시적 장애 방지를 위한 페일오버 실행 전 추가 대기 시간
3. **복구 확인**: 페일오버 실행 전 장애 노드가 여전히 다운 상태인지 확인
4. **자체 투표**: 생존 노드가 합의 달성을 위해 자동으로 자신에게 투표

#### 페어 모드 구성
```yaml
coordination:
  min_consensus_nodes: 1          # 페어 클러스터에서 자동 조정
  # 페어 모드 설정 (2노드 클러스터에서 자동 활성화)
  # pair_failover_delay: "10s"     # 페일오버 실행 전 안전 지연
  # pair_failure_threshold: 3      # 페일오버 전 필요한 연속 실패 횟수
```

📖 **자세한 페어 클러스터 구성은 [configs/pair-example.yaml](configs/pair-example.yaml)을 참조하세요**
